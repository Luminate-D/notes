...
watchEffect(() => {
  fetch(`https://api.lanny.dev/notes?limit=7&page=${page.value}`)
    .then(res => res.json())
...
<template>
  <div class="flex w-full h-dvh flex-col content-center items-center p-5">
    ...
    <div class="flex max-w-xl w-full p-5 gap-2">
      ...
      <button @click="addNote" type="button" class="text-white bg-blue-700 hover:cursor-pointer hover:bg-blue-800 focus:ring-blue-300 font-medium rounded-md text-sm px-4 py-2">Add</button>
    </div>
    <div class="flex flex-col h-full w-full content-center items-center">
      <div class="flex flex-row gap-5 mb-5">
        <p class="text-2xl hover:text-blue-600 hover:underline hover:cursor-pointer" @click="page > 1 ? page-- : void 0;">←</p>
        <p class="text-2xl">Page: {{ page }}</p>
        <p class="text-2xl hover:text-blue-600 hover:underline hover:cursor-pointer" @click="page++">→</p>
      </div>
